...
final class AnimalFactory extends ModelFactory
{
    ...
    {
        return [
            'name' => $name = self::faker()->lastname(),
            'poids' => self::faker()->randomNumber(2),
            'taille' => self::faker()->randomNumber(3), // en centimètres
            'habitat' => self::faker()->word(),
            'espece' => self::faker()->word,
            'esperanceVie' => self::faker()->randomNumber(2),
            'description' => "Le {$name} est un animal qui ...",
            'imageAni' => null,
        ];
    }
